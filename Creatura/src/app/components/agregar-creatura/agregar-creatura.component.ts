...
 }
 // MODIFICACIÓN DE LA FUNCIÓN VALIDAR RANGO
validarRango(event: Event, controlName: string) {
  const input = event.target as HTMLInputElement;
  let value = parseInt(input.value, 10);

  // Aplica la lógica de rango
...
  // El `emitEvent: false` es clave para evitar bucles infinitos.
  this.datosCreaturaForm.get(controlName)?.setValue(value, { emitEvent: false });
}
